...
def _stage_ts_file(repo: Path, relpath: str, body: str) -> Path:
    target = repo / relpath
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text(body)
    _git(repo, "add", "-A")
...
def test_ts_generated_file_is_exempt_from_the_size_cap(repo: Path) -> None:
    """A generated types module grows with the API it mirrors.

    `openapi-typescript` output carries an `AUTO-GENERATED` header and runs to
    thousands of lines; the size rule would otherwise block every API addition
    with a finding nobody can act on (nothing hand-written is there to split).
    The exemption is keyed on the leading comment only, so a hand-written module
    over the cap is still caught.
    """
    filler = "".join(f"export const g{i} = {i};\n" for i in range(1300))
    generated = _stage_ts_file(
        repo,
        "client/src/api.types.ts",
        "// AUTO-GENERATED by sync-api-types — do not edit\n" + filler,
    )

    result = _run_gate(TS_ORGANIZATION_GATE, repo, str(generated), scope="staged")

    assert result.returncode == 0, _out(result)

    # The marker only counts in the leading comment: a hand-written module of the
    # same size that mentions it further down is still over the cap.
    handwritten = _stage_ts_file(
        repo, "client/src/big_by_hand.ts", filler + "// see the AUTO-GENERATED types\n"
    )
    result = _run_gate(TS_ORGANIZATION_GATE, repo, str(handwritten), scope="staged")

    assert result.returncode == 1, _out(result)
    assert "big_by_hand.ts" in _out(result)
