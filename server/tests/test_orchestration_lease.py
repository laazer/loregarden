...
def test_a_run_never_renewed_falls_back_to_when_it_started(session, workspace):
    ...
    assert orchestration_lease_expired(_orch(session, _ticket(session, workspace), age=EXPIRED))
    assert not orchestration_lease_expired(_orch(session, _ticket(session, workspace, "T-2")))
