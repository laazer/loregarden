...
class TestParallelQueueEventEmissions:
    ...
    async def test_queue_run_emits_execution_update_when_queued(self, session):
    ...
    async def test_queue_run_emits_execution_update_when_started(self, session):
    ...
    async def test_queue_run_emits_error_on_failure(self, session):
        ...
        with patch('loregarden.services.parallel_queue.emit_error') as mock_error, \
            ...
            service = ParallelQueueService(session)
            result = await service.queue_run('ws-1', 'ticket-1', 'run-1')
    ...
    async def test_promote_from_queue_emits_queue_promoted(self, session):
    ...
    async def test_on_run_complete_emits_run_completed(self, session):
    ...
    async def test_on_run_complete_emits_execution_update(self, session):
    ...
    async def test_event_emission_graceful_failure(self, session):
...
class TestConflictDetectorEventEmissions:
    ...
    async def test_check_conflicts_emits_conflict_detected(self, session):
    ...
    async def test_merge_worktree_emits_conflict_resolved(self, session):
        """Verify conflict_resolved event emitted on successful merge."""
        # This tests the API endpoint, covered in integration tests
        assert True

    async def test_merge_worktree_emits_error_on_failure(self, session):
