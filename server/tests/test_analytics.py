...
class TestAnalyticsMetrics:
    ...
    async def test_metrics_per_ticket_type(self, db_session: Session):
        """Metrics grouped by ticket type."""
        # Should have separate entries for each ticket_type
        assert True

    async def test_duration_statistics(self, db_session: Session):
        """Calculate min/max/avg duration."""
        # Should compute from started_at to completed_at
        assert True

    async def test_success_rate_calculation(self, db_session: Session):
        """Calculate success rate per ticket type."""
        # completed runs / total runs
        assert True

    async def test_last_7_days_filtering(self, db_session: Session):
...
class TestAnalyticsTimeRanges:
    ...
    async def test_7_day_range(self, db_session: Session):
        """Get metrics for last 7 days."""
        # Should query runs from now - 7 days
        assert True

    async def test_30_day_range(self, db_session: Session):
        """Get metrics for last 30 days."""
        # Should query runs from now - 30 days
        assert True

    async def test_90_day_range(self, db_session: Session):
        """Get metrics for last 90 days."""
        # Should query runs from now - 90 days
        assert True

    async def test_invalid_range_rejected(self, db_session: Session):
...
class TestAnalyticsOrdering:
    ...
    async def test_sorted_by_frequency(self, db_session: Session):
        """Metrics sorted by count (most frequent first)."""
        # Ticket types with more runs appear first
        assert True

    async def test_empty_list_when_no_data(self, db_session: Session):
...
class TestAnalyticsErrors:
    ...
    async def test_invalid_workspace(self, db_session: Session):
        """Workspace that doesn't exist returns empty metrics."""
        # Should not error, just return empty list
        assert True

    async def test_no_completed_runs(self, db_session: Session):
...
class TestAnalyticsPerformance:
    ...
    async def test_query_performance(self, db_session: Session):
        """Analytics query completes quickly."""
        # Should be <500ms even with large dataset
        assert True

    async def test_large_dataset_handling(self, db_session: Session):
