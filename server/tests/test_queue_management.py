...
class TestQueueReordering:
    ...
    async def test_reorder_run_to_earlier_position(self, session: Session):
        ...
        await _reorder_queue_internal(session, "ws-1", "run-3", 3, 1)

        # Verify positions: run-3 should be at 1, run-1 at 2, run-2 at 3
        updated_run3 = session.exec(select(QueuedRun).where(QueuedRun.run_id == "run-3")).first()
        updated_run1 = session.exec(select(QueuedRun).where(QueuedRun.run_id == "run-1")).first()
        updated_run2 = session.exec(select(QueuedRun).where(QueuedRun.run_id == "run-2")).first()

        assert updated_run3.position == 1
        assert updated_run1.position == 2
        assert updated_run2.position == 3

    async def test_reorder_run_to_later_position(self, session: Session):
        ...
        await _reorder_queue_internal(session, "ws-2", "run-1", 1, 3)
    ...
    async def test_reorder_invalid_position_too_high(self, session: Session):
        ...
        with pytest.raises(HTTPException) as exc_info:
            ...
            await reorder_queued_run("run-1", 5, session)

        assert exc_info.value.status_code == 400
        assert "Invalid position" in exc_info.value.detail

    async def test_reorder_invalid_position_zero(self, session: Session):
        ...
        with pytest.raises(HTTPException) as exc_info:
            from loregarden.api.queue_management import reorder_queued_run
            await reorder_queued_run("run-1", 0, session)

        assert exc_info.value.status_code == 400

    async def test_reorder_non_existent_run(self, session: Session):
        """Test reordering a run that doesn't exist."""
        from fastapi import HTTPException

        with pytest.raises(HTTPException) as exc_info:
            from loregarden.api.queue_management import reorder_queued_run
            await reorder_queued_run("non-existent", 1, session)

        assert exc_info.value.status_code == 404

    async def test_reorder_non_queued_run(self, session: Session):
        ...
        with pytest.raises(HTTPException) as exc_info:
            from loregarden.api.queue_management import reorder_queued_run
            await reorder_queued_run("run-1", 1, session)

        assert exc_info.value.status_code == 400
        assert "not queued" in exc_info.value.detail

    async def test_reorder_same_position(self, session: Session):
        ...
        result = await reorder_queued_run("run-1", 1, session)

        assert result["status"] == "no_change"
        assert result["position"] == 1

    async def test_reorder_emits_execution_update(self, session: Session):
        ...
        with patch('loregarden.api.queue_management.emit_execution_update') as mock_emit:
            result = await reorder_queued_run("run-2", 1, session)
    ...
    async def test_reorder_preserves_other_workspace_queues(self, session: Session):
        ...
        session.add_all([run1_ws1, run2_ws1, run1_ws2, run2_ws2])
        session.commit()

        # Reorder in workspace 1
        await _reorder_queue_internal(session, "ws-8", "run-2-ws1", 2, 1)

        # Verify workspace 2 unchanged
        updated_run1_ws2 = session.exec(select(QueuedRun).where(QueuedRun.run_id == "run-1-ws2")).first()
        updated_run2_ws2 = session.exec(select(QueuedRun).where(QueuedRun.run_id == "run-2-ws2")).first()
...
class TestQueueInfo:
    ...
    async def test_get_queue_info_with_active_runs(self, session: Session):
        ...
        result = await get_queue_info("ws-10", session)
    ...
    async def test_get_queue_info_empty_queue(self, session: Session):
        ...
        result = await get_queue_info("ws-11", session)

        assert result["queue_length"] == 0
        assert result["max_position"] == 0
        assert result["runs"] == []

    async def test_get_queue_info_estimated_clear_time(self, session: Session):
        ...
        result = await get_queue_info("ws-12", session)

        # 3 queued runs * 300s = 900s
        assert result["estimated_clear_time_seconds"] == 900

    async def test_get_queue_info_invalid_workspace(self, session: Session):
        """Get info for non-existent workspace."""
        from loregarden.api.queue_management import get_queue_info

        result = await get_queue_info("non-existent-ws", session)
...
class TestQueuePromotion:
    ...
    async def test_promote_run_with_available_slot(self, session: Session):
        ...
        with patch('loregarden.api.queue_management.ParallelQueueService') as mock_service:
            ...
            result = await promote_run("run-1", session)

            assert result["status"] == "promoted"
            assert "slot_number" in result["message"] or "promoted_run" in result

    async def test_promote_run_no_available_slots(self, session: Session):
    ...
    async def test_promote_non_existent_run(self, session: Session):
        """Try to promote run that doesn't exist."""
        from fastapi import HTTPException

        with pytest.raises(HTTPException) as exc_info:
            from loregarden.api.queue_management import promote_run
            await promote_run("non-existent", session)

        assert exc_info.value.status_code == 404

    async def test_promote_non_queued_run(self, session: Session):
...
class TestQueueErrorHandling:
    ...
    async def test_reorder_websocket_emit_failure(self, session: Session):
    ...
    async def test_queue_info_handles_missing_runs(self, session: Session):
        ...
        result = await get_queue_info("ws-17", session)
...
class TestQueueConcurrency:
    ...
    async def test_concurrent_reorder_requests(self, session: Session):
        ...
        async def reorder_task(run_id, old_pos, new_pos):
            await _reorder_queue_internal(session, "ws-18", run_id, old_pos, new_pos)
        ...
        all_runs = session.exec(select(QueuedRun).where(QueuedRun.workspace_id == "ws-18").order_by(QueuedRun.position)).all()
        positions = [run.position for run in all_runs]

        # Should have consistent positions 1, 2, 3
        assert sorted(positions) == [1, 2, 3]

    async def test_multiple_workspaces_concurrent_reorder(self, session: Session):
        ...
        await _reorder_queue_internal(session, "ws-19", "run-2-ws-19", 2, 1)
        await _reorder_queue_internal(session, "ws-20", "run-2-ws-20", 2, 1)

        # Verify each workspace is correctly reordered independently
        runs_ws1 = session.exec(select(QueuedRun).where(QueuedRun.workspace_id == "ws-19").order_by(QueuedRun.position)).all()
        runs_ws2 = session.exec(select(QueuedRun).where(QueuedRun.workspace_id == "ws-20").order_by(QueuedRun.position)).all()
...
class TestQueueIntegration:
    ...
    async def test_reorder_then_complete_run(self, session: Session):
        ...
        await _reorder_queue_internal(session, "ws-21", "run-3", 2, 1)
    ...
    async def test_reorder_affects_queue_state(self, session: Session):
