...
**Loregarden control plane (when SQLite is authoritative):** The top-level orchestrator owns workflow transitions. Sub-agents must **not** edit ticket WORKFLOW STATE in markdown and must **not** call `loregarden_complete_stage` — they emit `<<<LOREGARDEN_STAGE_REPORT>>>` and you advance from that. After each stage (and after any transition gates pass), call Loregarden MCP tools:
...
- `loregarden_complete_stage` — advance after sub-agent stage report + gates succeed (or `outcome: reject` when the sub-agent reported fail/needs_rework)
...
A sub-agent that exits cleanly with **no** stage report is a blocked stage — do not invent a pass.
